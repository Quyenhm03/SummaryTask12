...
fun initData(inventory: InventoryRepository, orderRepo: OrderRepository) {
    ...
    val laptop = Electronics("LAP1", "Dell", 13500000.0, 2)
...
fun addElectronic(inventory: InventoryRepository) {
    ...
    try {
        val product = if (warranty != null) {
            Electronics(id, name, price, warranty)
        } else {
            Electronics(id, name, price)
        }
...
fun reportsAndStatistics(inventory: InventoryRepository, orderRepo: OrderRepository, currentRevenue: Double) {
    while (true) {
